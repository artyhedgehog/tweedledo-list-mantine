...
export function DataPage() {
    ...
    const form = useForm({
        ...
        initialValues: {
            data: yaml.stringify(
                Utils.getAllData(lists, config, configNamespace)
            ),
        },
        ...
        transformValues: ({ data }) =>
            data === yamlRef.current.value
                ? yamlRef.current.parsed
                : yaml.parse(data),
    });
    ...
    const handleSave = (data: Record<string, unknown>) => {
        try {
            ...
            form.resetDirty();
            notifications.show({
                message: 'Data from the YAML is imported to the app',
            });
        } catch (e: any) {
                ...
                <form onSubmit={form.onSubmit(handleSave, handleError)}>
                    ...
                    <Group gap={5} mt={5}>
                        ...
                        <Button
                            ...
                            type="submit"
                            disabled={
                                !form.isDirty('data') || !dataInputProps.value
                            }
